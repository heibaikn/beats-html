...
<template>
  <section>
    <div style="padding:5px 0 10px;" v-if="checkAdminIdentity">
      <form
        action="/api/admin/customerMessage/exportCustomerMessages"
        target="_blank"
        id="exportForm"
        method="post"
      >
        ...
        <Form :label-width="200">
          ...
          <FormItem label="浏览的产品：">
            <div class="msg-product-list">
              <span
                v-for="(item,index) in modal3Data.goodsSpus"
                :key="index"
                @click="lookProduct(item)"
              >
...
export default {
  ...
  methods: {
    ...
    queryList() {
      this.loading = true;
      this.api
        .getCustomerMessages(this.formSearch)
        .then(d => {
          this.loading = false;
